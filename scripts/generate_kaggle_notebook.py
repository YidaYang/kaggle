...
def build_cells() -> list[dict]:
    ...
    cells.append(make_code(
        ...
        "EPOCHS           = 3\n"
        "BATCH_SIZE       = 2        # T4 16GB 推荐 2\n"
        "GRAD_ACCUM_STEPS = 8\n"
        "MAX_LENGTH       = 1024     # T4 16GB 推荐 1024\n"
        "LEARNING_RATE    = 2e-4\n"
        "USE_LORA         = True\n"
    ...
    cells.append(make_markdown(
        ...
        "| --- | --- | --- |\n"
        "| `BATCH_SIZE` | 2 | T4 16GB 推荐 2 |\n"
        "| `GRAD_ACCUM_STEPS` | 8 | 有效 batch = BATCH_SIZE × GRAD_ACCUM_STEPS |\n"
        "| `MAX_LENGTH` | 1024 | 输入序列最大 token 数 |\n"
        "| `EPOCHS` | 3 | 训练轮数 |\n"
        "| `LEARNING_RATE` | 2e-4 | AdamW 学习率 |\n"
        "| `LOAD_IN_4BIT` | True | 4-bit NF4 量化 (QLoRA) |"
    ))
    cells.append(make_code(
        "import sys\n"
        "\n"
